...
import { range, sample, isOdd, isEven } from 'lodash';
...
const useGenerateNumbers = () => {
    const generateNumbers = (lotteryName) => {
        ...
        const numbers = []
        const megaball = sample(possibleMegaballNumbers.filter((n) =>  n % 2 === 1 ))
        ...
        while (numbers.length < 5) {
            ...
            if (number % 2 === 1 && oddCounter < 2) {
                numbers.push(number);
                oddCounter += 1;
            } else if (number % 2 === 0 && evenCounter < 3) {
                numbers.push(number);
                evenCounter += 1;
            };
        };
